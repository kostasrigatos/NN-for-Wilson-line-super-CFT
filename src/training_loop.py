...
def train_and_save(lam: float, g_values: list[float], design_matrix_cache: dict[float, torch.Tensor], target_vector_cache: dict[float, torch.Tensor], n_epochs: int = 30000, seed: int = 42, save: bool = True) -> tuple[nn.Module, list[float]]:
    r"""
...
def train_seed_sweep_parallel(lams, seeds, n_epochs=30000, max_workers=10):
    r"""
          ...
          and timing data, into a single list with no change to model architecture across jobs.

      """
    jobs = [(lam, seed, n_epochs) for lam in lams for seed in seeds]
